...
abstract class TestDotsAndBoxes {
    ...
    abstract fun createGame(
        columns: Int = 8,
        rows: Int = 8,
        players: List<Player> = listOf(TestHumanPlayer(), TestHumanPlayer())
    ): DotsAndBoxesGame
    ...
    fun testPlayerList(playerCount: Int) {
        val players = List(playerCount) { TestHumanPlayer() }
    ...
    fun testPlayerListDisconnected(playerCount: Int) {
        val players = MutableList(playerCount) { TestHumanPlayer() }
        val game = createGame(players = players)
    ...
    fun testMoveTwiceDisallowed(width: Int, height: Int, rnd: Random) {
        ...
        run {
            ...
            assertTrue(
                line.isDrawn,
                "After drawing a line, its isDrawn property should now be true"
            )
    ...
    fun testMakeMove(width: Int, height: Int, rnd: Random) {
        ...
        assertEquals(
            false,
            origLines[lineToPlay.coordinates].isDrawn,
            "A game just created should not have any drawn line"
        )
        ...
        assertEquals(
            origLines.size,
            game.lines.count(),
            "The amount of lames in a game should be constant"
        )
        ...
        assertEquals(
            true,
            newLine.isDrawn,
            "The line returned from lines with the drawn coordinates should have the isDrawn property set, even if it is not the exact same object"
        )
        // There should be exactly one drawn line in th game
        assertEquals(
            1,
            game.lines.count { it.isDrawn },
            "A game with one move should have exactly 1 line drawn"
        )
        // Playing the single line cannot complete a box, so no repeat turns. New player is needed
        assertNotEquals(
            origPlayer,
            game.currentPlayer,
            "After a single move, the first player cannot be the current player as a repeat move requires a completed box (at least 3 lines for triangles, 4 for rectangles etc.)"
        )
        // Check that the playing triggered the listener exactly once for a game state change
        assertEquals(
            1,
            gameListener.onGameChangeCalled,
            "The game change listener should have been invoked exactly once"
        )
        // Check that the game over listener was not called
        assertFalse(
            gameListener.onGameOverCalled,
            "The game cannot be complete after a single move"
        )
    ...
    fun testGamePlayBox(width: Int, height: Int, rnd: Random) {
        ...
        for (lineCoordinate in lineCoordinatesToPlay) {
            run {
                ...
                val lineFromBox =
                    box.boundingLines.singleOrNull { it.coordinates == lineCoordinate }
                        ?: fail("There should be a single line surrounding the box with the expected coordinates")
                assertNull(box.owningPlayer, "When starting a game, no box should have an owner")
                assertEquals(
                    lineCoordinate,
                    lineFromGame.coordinates,
                    "The coordinates of a line should match those used to retrieve it (in DotsAndBoxesGame.lines)"
                )
                assertEquals(
                    lineCoordinate,
                    lineFromBox.coordinates,
                    "When getting the box again, there should be exactly 1 line in the bounding lines of the box with the coordinates"
                )
                assertFalse(
                    lineFromBox.isDrawn,
                    "This game draws its own lines, so the line should not be magically become drawn when getting it through the box"
                )
                assertFalse(
                    lineFromGame.isDrawn,
                    "This game draws its own lines, so the line should not be magically become drawn when getting it from DotsAndBoxesGame.line"
                )
            ...
            run {
                ...
                assertTrue(
                    lineFromBox.isDrawn,
                    "After drawing a line it should be drawn when retrieving it as bounding line of a box"
                )
                assertTrue(
                    lineFromGame.isDrawn,
                    "After drawing a line it should be drawn when retrieving it from DotsAndBoxesGame.lines"
                )
            }
        }
        run {
            ...
            assertNotNull(
                playedBox.owningPlayer,
                "When all bounding lines of a box are drawn, the box should now have an owning player"
            )
            for (line in playedBox.boundingLines) {
                assertTrue(
                    line.isDrawn,
                    "All the lines that bound the box should have their isDrawn property as true."
                )
            }
            for (lineCoordinate in lineCoordinatesToPlay) {
                assertTrue(
                    game.lines[lineCoordinate].isDrawn,
                    "For the bounding line coordinates, retrieving the lines with those coordinates should have drawn lines"
                )
            }
        }
    ...
    fun testRemoveGameChangeListener(width: Int, height: Int, rnd: Random) {
        ...
        assertEquals(
            1,
            gameListener1.onGameChangeCalled,
            "The 1st gameChangeListener should have been called exactly once"
        )
        assertEquals(
            1,
            gameListener2.onGameChangeCalled,
            "The 2nd gameChangeListener should have been called exactly once"
        )
        // Game over should not have been called
        assertFalse(
            gameListener2.onGameOverCalled,
            "The game over listener should not have been called at all"
        )
        ...
        run {
            ...
            assertEquals(
                2,
                gameListener1.onGameChangeCalled,
                "After the 2nd gameChangeListener has been removed, the first one should have still been called on a drawLine"
            )
            assertEquals(
                1,
                gameListener2.onGameChangeCalled,
                "After the 2nd gameChangeListener has been removed, it should not be called on a drawLine"
            )
        }
        ...
        run {
            ...
            assertEquals(
                2,
                gameListener1.onGameChangeCalled,
                "After removing the 1st gameChnageListener it should not be called on a drawLine"
            )
            assertEquals(
                2,
                gameListener2.onGameChangeCalled,
                "After re-adding the 2nd gameChangeListener (and removing the 1st), it should be called again on drawLine"
            )
        }
    }
    ...
    fun testCompleteGame(width: Int, height: Int, rnd: Random) {
        ...
        for (lineCoord in lineCoordinatesToPlay) {
            ...
            assertFalse(
                line.isDrawn,
                "Lines should not just turn up as drawn as the coordinates for lines are supposed to be unique"
            ) // It can not have been drawn yet
            ...
            assertFalse(
                game.isFinished,
                "Until the last line has been drawn, the game cannot be complete/finished"
            ...
            for (neighbor in game.lines[lineCoord].adjacentBoxes) {
                if (neighbor.boundingLines.all(Line::isDrawn)) {
                    assertEquals(
                        player,
                        neighbor.owningPlayer,
                        "If a neighboring box of a just drawn line has all its lines drawn, the owner of the box should be the player that was the current player before calling drawLine"
                    )
                } else {
                    assertNull(
                        neighbor.owningPlayer,
                        "If a box doesn't have all its lines drawn it should not have an owner"
                    )
                }
            }
            ...
            assertFalse(
                gameListener2.onGameOverCalled,
                "The game isn't finished, so gameOver should not have been called on listener 1"
            )
            assertFalse(
                gameListener1.onGameOverCalled,
                "The game isn't finished, so gameOver should not have been called on listener 2"
            )
            // Check that onGameChange was called
            assertNotEquals(
                lastGameChangeCount,
                gameListener1.onGameChangeCalled,
                "onGameChange should have been called on listener 1"
            )
            assertNotEquals(
                lastGameChangeCount,
                gameListener2.onGameChangeCalled,
                "onGameChange should have been called on listener 2"
            )
            assertEquals(
                gameListener1.onGameChangeCalled,
                gameListener2.onGameChangeCalled,
                "both game change listeners should have been called with the same frequency"
            )
        ...
        val adjacents =
            lastLineToPlay.adjacentBoxes.run { first?.coordinates to second?.coordinates }
        for (box in game.boxes) {
            if (box.coordinates in adjacents) {
                assertEquals(
                    1,
                    box.boundingLines.count { !it.isDrawn },
                    "The boxes adjacent to the last undrawn line should only have 1 undrawn line"
                )
                assertNull(
                    box.owningPlayer,
                    "THe boxes adjacent to the last undrawn line should not have an owner"
                )
            } else {
                assertTrue(
                    box.boundingLines.all { it.isDrawn },
                    "A box not adjacent to the last undrawn line should have all its lines drawn"
                )
                assertNotNull(
                    box.owningPlayer,
                    "A box not adjacent to the last undrawn line should have an owning player"
                )
            }
        }
        ...
        assertTrue(
            game.isFinished,
            "After drawing the last undrawn line the game should be finished."
        )
        ...
        if (isDropFirstListener) {
            assertEquals(
                lastGameChangeCount,
                gameListener1.onGameChangeCalled,
                "When listener 1 is unregistered, it should not be called on the last game play"
            )
            assertNotEquals(
                lastGameChangeCount,
                gameListener2.onGameChangeCalled,
                "When listener 1 is unregistered, listener 2 should still be informed of game change on the last play"
            )
            assertFalse(
                gameListener1.onGameOverCalled,
                "When listener 1 is unregistered, it should not be informed of game completion"
            )
            assertTrue(
                gameListener2.onGameOverCalled,
                "When listener 1 is unregistered, listener 2 should still be informed of game completion"
            )
        } else {
            assertNotEquals(
                lastGameChangeCount,
                gameListener1.onGameChangeCalled,
                "When listener 2 is unregistered, listener 1 should still be informed of game change on the last play"
            )
            assertEquals(
                lastGameChangeCount,
                gameListener2.onGameChangeCalled,
                "When listener 2 is unregistered, it should not be called on the last game play"
            )
            assertTrue(
                gameListener1.onGameOverCalled,
                "When listener 2 is unregistered, listener 1 should still be informed of game completion"
            )
            assertFalse(
                gameListener2.onGameOverCalled,
        ...
        assertNotEquals(
            gameListener1.onGameChangeCalled,
            gameListener2.onGameChangeCalled,
            "The two listeners should not have been called about game changes the same amount of time"
        )
        ...
        for (c in adjacents) {
            ...
            assertEquals(
                lastPlayerToPlay,
                b.owningPlayer,
                "The boxes adjacent to the last line should have the last player as owner, box $b at $c does not"
            )
            assertTrue(
                b.boundingLines.all(Line::isDrawn),
                "All bounding lines of the adjacent boxes to the last line should be drawn"
            )
        }

        // In general all lines should be drawn and all boxes owned
        assertTrue(
            game.lines.all(Line::isDrawn),
            "All lines in the game/returned by DotsAndBoxesGame.lines should be drawn as the game is finished"
        )
        assertTrue(
            game.boxes.all { it.owningPlayer != null },
            "All boxes in the game/returned by DotsAndBoxesGame.boxes should have an owner"
        )
    }
    ...
    fun testComputerGame(width: Int, height: Int, rnd: Random) {
        ...
        val players = listOf(TestHumanPlayer(), computerPlayer)
        ...
        while (lineCoordinatesToPlayIterator.hasNext()) {
            ...
            for (b in lineToPlay.adjacentBoxes) {
                assertNull(
                    b.owningPlayer,
                    "The adjacent boxes to a line to be played should not have an owner"
                )
            ...
            for (bcoord in boxesThatWillComplete) {
                ...
                if (b.boundingLines.all { it.isDrawn }) {
                    ...
                    assertEquals(
                        players[0],
                        b.owningPlayer,
                        "For all boxes that had 1 undrawn line, check that they are now owned by the human player"
                    )
                } else {
                    assertNull(
                        b.owningPlayer,
                        "If boxes are not drawn they should not have an owner"
                    )
            ...
            if (didCompleteBox) {
                ...
                assertEquals(
                    oldComputerTurns,
                    computerPlayer.computerTurns,
                    "If the human player completes a box, the computer player should not get a turn"
                )
                assertEquals(
                    drawnLines + 1,
                    game.lines.count { it.isDrawn },
                    "the amount of lines expected to be drawn should equal the amount of lines actually drawn"
                )
            } else {
                // Computer gets one or multiple turns
                assertTrue(
                    oldComputerTurns < computerPlayer.computerTurns,
                    "The amount of computer turns should be at least 1 more than previously"
                )
                assertTrue(
                    drawnLines + 1 < game.lines.count { it.isDrawn },
                    "The amount of actually drawn line needs to be at least 2 more than previous (1 human turn, 1+ computer turns)"
                )
            }
        }
        assertEquals(
            game.lines.count(),
            humanTurns + computerPlayer.computerTurns,
            "The total amount of turns by the computer + the total amount of turns by the \"human\" should equal the actual amount of lines"
        )
    }
    ...
    companion object {
        ...
        fun gameMoveData(): List<Array<Any>> {
            ...
            return sizes.flatMap { size ->
                (1..size[2]).asSequence()
                    .map { arrayOf(size[0], size[1], Random(seedSource.nextInt())) }
            }.toList()
        }
    }
...
class TestComputerPlayer : ComputerPlayer {
    ...
    override fun makeMove(game: DotsAndBoxesGame) {
        assertTrue(
            ...
            "There should still be a move to play when the computer player is asked to make a move (was isFinished updated correctly?)"
        )
        val line = game.lines[moveIterator.next()]
        line.drawLine()
        assertTrue(
            game.lines[line.coordinates].isDrawn,
            "After the computer draws a line, the line at that coordinate should be drawn"
        )
        computerTurns++
        for (b in line.adjacentBoxes) {
            if (b.boundingLines.all { it.isDrawn }) {
                assertEquals(
                    this,
                    b.owningPlayer,
                    "If the computer move completed a box, the computer player should be the owner"
                )
            } else {
                assertNull(
                    b.owningPlayer,
                    "If the computer move didn't complete a box, the box should not have an owner"
                )
            }
        }
    }
 ...
 */
class TestGameListener<G : DotsAndBoxesGame>(val expectedGame: G) :
    DotsAndBoxesGame.GameChangeListener,
